...
//		testGetPerson();
	...
	private static void testGetPerson() {
		...
		//we can't get List<Employee> because JSON convertor doesn't know the type of
		//object in the list and hence convert it to default JSON object type LinkedHashMap
		Person person = restTemplate.getForObject(SERVER_URI+"/rest/person/7", Person.class);
			System.out.println("ID="+person.getId()+",Name="+person.getName()+",Country="+person.getCountry());;
	}

	private static void testCreateEmployee() {
		RestTemplate restTemplate = new RestTemplate();
		Employee emp = new Employee();
		emp.setId(1);emp.setName("Joan Martinez");
